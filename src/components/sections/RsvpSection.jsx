...
import {
  buildFallbackRsvpMessage,
  openWhatsAppMessage,
} from "../../utils/whatsapp";

const ENABLE_RSVP_WHATSAPP_FALLBACK =
  import.meta.env.VITE_INCIDENT_RSVP_FALLBACK_ENABLED !== "false";
...
export default function RsvpSection() {
  ...
  function handleFallbackSubmitViaWhatsApp() {
    const message = buildFallbackRsvpMessage(form);
  ...
  return (
    <section
      id="rsvp"
      className="py-24 sm:py-32 px-6 bg-ink relative overflow-hidden"
    >
      ...
      <div className="relative z-10 max-w-xl mx-auto">
        ...
        <div className="mb-14 sm:mb-16">
          ...
          <p className="mt-4 text-[15px] sm:text-base text-ink-faint leading-relaxed max-w-sm">
            Kami memohon kesediaan Bapak/Ibu untuk melengkapi data berikut guna
        ...
        {submitted && submittedData ? (
          ...
          <div className="animate-fade-up bg-ink-light p-8 rounded-[32px] border border-white/5">
            ...
            <p className="text-[15px] text-ink-faint mb-10">
              Data kehadiran Bapak/Ibu telah berhasil dicatat ke dalam sistem.
            </p>
            ...
            <div className="bg-white/[0.03] rounded-2xl border border-white/[0.05] p-6 text-left space-y-4 mb-10">
              {[
                ...
                <div
                  key={item.label}
                  className="flex justify-between items-center pb-4 border-b border-white/[0.05] last:border-0 last:pb-0"
                >
                  <span className="text-[13px] font-medium text-ink-faint">
                    {item.label}
                  </span>
                  <span className="text-[14px] font-bold text-white text-right break-words max-w-[60%]">
                    {item.value}
                  </span>
                </div>
              ))}
          ...
          >
            ...
            <div className="group">
              ...
              >
                ...
                {EVENT_CONFIG.units.map((u) => (
                  <option key={u} value={u} className="text-ink bg-paper-white">
                    {u}
                  </option>
                ))}
              </select>
            ...
            {ENABLE_RSVP_WHATSAPP_FALLBACK && (
              <button
                type="button"
                onClick={handleFallbackSubmitViaWhatsApp}
                className="w-full h-14 flex items-center justify-center gap-2 px-6 bg-white/5 text-white text-[14px] font-bold rounded-2xl border border-white/15 hover:bg-white/10 transition-all duration-300 active:scale-[0.98]"
              >
                <MessageCircle size={18} />
                Kirim RSVP Manual via WhatsApp
              </button>
            )}

            {ENABLE_RSVP_WHATSAPP_FALLBACK && (
              <p className="text-xs text-ink-faint text-center leading-relaxed">
                Gunakan opsi ini jika form online terkendala jaringan atau
                server.
              </p>
            )}
          </form>
